...
1. 空闲盘块的组织
	+ 空闲盘块号栈,用来存放当前可用的一组空闲盘块的盘块号+栈中尚有的空闲盘块号数目N
		- N兼做栈顶指针使用
		- 临界资源,每次只允许一个进程访问,设置有一把锁
	+ 文件区中的所有空闲盘块被分成若干个组
	+ 将每一组含有的盘块总数N和该组所有盘块号进入其前一组的第一个盘块中
		- 各组的第一个盘块链成了一条链
	+ 将第一组的盘块总数和盘块号计入空闲盘块号栈中,作为当前可供分配的空闲盘块号
	+ 最末一组有空闲盘块链的结束标志
2. 空闲盘块的分配与回收
	+ 分配:
		- 检测是否上锁
			+ 如未上锁,从栈顶取出一空闲盘块号将与之对应的盘块分配给用户
				- 然后将栈顶指针下移一格,->检查是否到了栈底
					+ <u>**若到了栈低,调用磁盘读过程将栈底盘块号所记录的下一组盘块信息读入栈中**</u>
					作为新的盘块号栈的内容
			+ ,????栈低时的细节,,,
	+ 回收:
		- 调用盘块回收过程,将回收盘块的盘块号计入空闲盘块号栈的顶部,执行空闲盘块号数目加1操作
			+ 如果空闲盘块号数目达到了100即栈满,将现有栈中的100个盘块号记入新回收的盘块中,
			将其盘块号作为新的栈底
###8.3提高磁盘I/O速度的途径
###8.4提高磁盘可靠性的技术
###8.5数据一致性控制
